  ...
  <div class="row">
    ...
    @if(isset($archiviate))
      <div class="alert alert-info">
        <i class="fa fa-info-circle"></i> Nessuna offerta <em>archiviata</em>.
      </div>
    @else
      <div class="alert alert-info">
        <i class="fa fa-info-circle"></i> Non hai ancora creato nessuna <em>Offerta</em>, <a href="{{ url("admin/offerte/create") }}">creane una ora</a>.
      </div>
    @endif
    </div>
  </div>
        ...
        <thead>
          <tr>
            ...
            <th class="text-center" rowspan="4" valign="center">
              <a href="{{ url('admin/offerte/'.$offerta->id.'/edit')}}" class="btn btn-primary">Modifica</a>
            </th>
            <th class="text-center" rowspan="4" valign="center">
              
              @if (isset($archiviate))
                  {!! Form::open(['id' => 'record_delete_'.$offerta->id, 'url' => 'admin/offerte/'.$offerta->id, 'method' => 'DELETE']) !!}
                  ...
                  {!! Form::open(['id' => 'archvia', 'url' => 'admin/offerte/'.$offerta->id.'/ripristina', 'method' => 'POST']) !!}
                    <button type="submit" class="btn btn-green">Pubblica</button>
                  {!! Form::close() !!}
                @else
                  {!! Form::open(['id' => 'archvia', 'url' => 'admin/offerte/'.$offerta->id.'/archivia', 'method' => 'POST']) !!}
