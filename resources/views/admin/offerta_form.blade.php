...
<div class="row">
  <div class="col-lg-12">
    ...
    @if (isset($offerta))
      {!! Form::open(['id' => 'record_delete', 'url' => 'admin/offerte/'.$offerta->id, 'method' => 'DELETE']) !!}
   ...
   @if (isset($offerta))
      ...
      {!! Form::model($offerta, ['role' => 'form', 'route'=>['offerte.update',$offerta->id], 'method' => 'PUT', 'class' => 'form-horizontal', 'id' => 'form_modifica_offerta','files' => true]) !!}
    ...
    @else
      ...
      {!! Form::open(['role' => 'form', 'route'=>['offerte.store'], 'method' => 'POST', 'class' => 'form-horizontal', 'files' => true]) !!}
       ...
       @if (isset($offerta) && Auth::user()->hasRole(["root","admin", "operatore"]) )
        ...
        <div class="form-group" @if($hotel->tipologia_id != "2") style="display:none;" @endif>
            ...
            <div class="col-sm-10">
	            ...
	            <span class="dal" style="float:none; display: inline; "> &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;* Se marcato come "No" il prezzo non è più da considerarsi a persona al giorno ma a pacchetto</span>
            </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-3">
               {!! Form::select('per_giorni',$giorni,isset($offerta) ? $offerta->per_giorni : 6,["class"=>"form-control"]) !!}
            </div>
        </div>
            ...
            @else
                @if (isset($offerta))
                    ...
                    @if (isset($showArchivia) && $showArchivia == 1)
                          ...
                          <div class="form-group" style="margin-top:20px;">
                            <textarea class="form-control" rows="2" placeholder="Specifica qui ulteriori motivazioni dell'archiviazione e verranno aggiunta alla mail" name="note" id="note">@if ( old('note') != '' ){{ old('note') }} @else @if(isset($offerta)){{$offerta->note}} @else ' ' @endif @endif
                            </textarea>
                          </div>
                    @endif
...
@section('onbodyclose')
  ...
  <script type="text/javascript">
    ...
    <?php /*
      ...
      jQuery("#valido_dal").val(start.format("DD/MM/YYYY"));
      jQuery("#valido_al").val(end.format("DD/MM/YYYY"));
      }
      ...
      var chi = jQuery(this).data("id");
      var node = jQuery("#" + chi + " span");
      node.html("Seleziona le date");

      jQuery("#valido_dal").val("");
      jQuery("#valido_al").val("");		    
