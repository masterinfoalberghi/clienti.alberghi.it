...
@section('title')
Statistiche mail dirette da scheda
@endsection
...
{!! Form::open(array('action' => 'Admin\StatsMailSchedaController@schede', 'class' => 'form-horizontal')) !!}
		  ...
		  <div class="form-group">
			...
			<div class="col-sm-3">
  		    ...
  		    <div class="col-sm-3">
				...
				{!! Form::select('tipologia', ['-1' => 'Tutte'] + $data['tipologie'] + ["-2" => 'Doppie (bloccate)'], null, array('class' => 'form-control' ) ) !!}
		    </div>
							...
							<tbody>
								...
								@foreach($mese_stats as $giorno_k_stats => $hotel_count)
									...
									@php 
										...
										$hotel_count_row_tot = $hotel_count["normale"] + $hotel_count["mobile"] + $hotel_count["doppia"]; 
									@endphp
	...
	@if($stats_chart)
		...
		<script type="text/javascript">
			...
			function drawChart() {
				...
				data.addColumn('number', 'Doppie (bloccate)');
				...
				@foreach($stats_chart as $anno_k_stats => $anno_stats)
					@foreach($anno_stats as $mese_k_stats => $mese_stats)
						@foreach($mese_stats as $giorno_k_stats => $hotel)
							...
							data.addRows([
							 	['{{$giorno_k_stats}} {{$mesi[intval($mese_k_stats)]}}', {{$hotel["normale"]}}, {{$hotel["mobile"]}}, {{$hotel["doppia"]}}],
							]);
