				...
				@if (!is_null($offerteTopLingua))
					...
					<a name="{{$offerteTop->id}}" id="{{$offerteTop->id}}" ></a>
						...
						<article class="offerta-container">
									...
									<div class="offerta-container-info">
										...
										<ul>
											...
											@if(isset($offerteTop->valido_dal))
												...
												<li><small>
													<b>{{ trans('hotel.valido_da_al_1') }}</b> {{Utility::getLocalDate($offerteTop->valido_dal, '%d %b')}}
							...
							<header>
								<h1>{{ strtoupper($offerteTopLingua->titolo)}}</h1>
							</header>				
							...
							<a data-id="{{$cliente->id}}" data-cliente="{{$cliente->nome}}" class="pulsante_chiama_schedabutton button green white-fe small callbutton" href="tel:{{explode(',',$cliente->telefoni_mobile_call)[0]}}">
								{{trans("hotel.chiama")}}
							</a>
			...
			@foreach ($offers as $offerteLast)
				...
				@if (!is_null($offerteLastLingua))
					...
					<article class="offerta-container"> {{-- position:relative per label approvazione --}}
						<div class="row offerta-container-row">
								...
								<div class="offerta-container-info">
									...
									<ul>
										...
										@if(isset($offerteLast->valido_dal))
											<li><small>
												<b>{{ trans('hotel.valido_da_al_1') }}</b> {{Utility::getLocalDate($offerteLast->valido_dal, '%d %b')}} {{ trans('hotel.valido_da_al_2') }} {{Utility::getLocalDate($offerteLast->valido_al, '%d %b')}}
											</small></li>
										@endif
							...
							<div class="col-xs-4">
								<div class="offerta-container-price">
									...
									<strong style="display:inline" class="price">{{$offerteLast->prezzo_a_persona}} &euro;</strong>
								</div>
							</div>
						</div>
						
						<header>
							<h1>{{strtoupper($offerteLastLingua->titolo)}}</h1>
						</header>
						
						<span class="testohotel" id="offerte-{{$offerteLast->id}}">
						...
						{!! Form::hidden('testo_offerta', trans("hotel.offerte_generiche_box" ))!!}
						{!! Form::hidden('tipo_offerta', "os" )!!}
						{!! Form::submit(trans("hotel.scrivi"),["class"=>"invioemailhotel button orange emailbutton small"]) !!}	
						{!! Form::close() !!}
