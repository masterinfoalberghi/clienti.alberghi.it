	...
	<article data-id="{{$cliente->id}}" data-url="{{$url_hotel}}" data-categoria="{{$cliente->categoria_id}}" data-prezzo="{{$cliente->prezzo_min}}" data-lat="{{$cliente->mappa_latitudine}}" data-lon="{{$cliente->mappa_longitudine}}" class="draw_item_cliente_vaat_listing link_item_slot link_item_slot_count @if(isset($class_item)) {{$class_item}} @endif">
		...
		<div class="col-text-draw">
			...
			<a class="linktextoffer no_upper bambini-gratis-listing" href="{{$url}}/{{$vaat->offerta->id}}">
			...
			@if (!is_null($altre_offerte) && $altre_offerte->count())
				@foreach ($altre_offerte as $bg)
					<a class="linktextoffer no_upper bambini-gratis-listing" href="{{$url_hotel}}&children-offers#{{$bg->id}}">
						<b class="upper title-offer-listing">{{strtoupper(trans('hotel.gratis_fino_a'))}} <span class="rosso">{{strtoupper($bg->_fino_a_anni())}} {{strtoupper($bg->_anni_compiuti())}}</span></b><br />
