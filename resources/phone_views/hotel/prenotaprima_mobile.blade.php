...
<html lang="{{$locale}}">
  <head>
	...
	<style>
		...
		@include('mobile.css.css-above.above-scheda-mobile-pp')
	</style>
  ...
  <body class="page-scheda info">
	...
	<header class="hidden" role="banner">
		<h1>{{trans("title.offerte_scheda")}}</h1>
	</header>
	
	@include('menu.header_menu_scheda') 
	
	@if ( $cliente->offertePrenotaPrima->count() < 1 && $cliente->offerteTop->count() < 1 && $cliente->last->count()< 1 && $cliente->offerte->count() < 1 )
			...
			<div class="container">
					...
					<div class="row white-bg" style="padding-top:50px;">
						<div class="col-xs-12"  id="datihotel" style="text-align:center; ">
							...
							@include("share" , ["marginbottom"=>1, "text" => 'Ci sono delle offerte *prenota prima e risparmia* per questo hotel a *'.$selezione_localita.'*'])
						</div>
					</div>
								
					@include('composer.offertePrenotaPrima', array('titolo' => trans('hotel.offerte_prenota_prima_box')))
