...
$storeFolder = $_POST['path']; // ../../../../../doc/

$first_dir_upload = "rassegna";
$second_dir_upload = "img_testi";
...
if (!empty($_FILES) && $upload_files) {
    ...
    $targetPath = dirname( __FILE__ ) . $ds. $storeFolder . $ds . $first_dir_upload. $ds; 
    $secondTargetPath = dirname( __FILE__ ) . $ds. $storeFolder . $ds . $second_dir_upload. $ds; 

    $targetPathThumb = dirname( __FILE__ ) . $ds. $storeFolderThumb . $ds; 
     
    $targetFile =  $targetPath. $_FILES['file']['name']; 
    $secondTargetFile =  $secondTargetPath. $_FILES['file']['name']; 

    $targetFileThumb =  $targetPathThumb. $_FILES['file']['name']; 
    
    if(move_uploaded_file($tempFile,$targetFile))
    	{
    	copy($targetFile,  $secondTargetFile);
    	}
