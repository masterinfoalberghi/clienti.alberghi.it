...
class LastRequest extends Request
  {
  ...
  private function _check_dates(&$rules)
    {
      ...
      // valido_al entro 1 mesi da adesso 
      $rules["valido_al"][] = "entro_giorni_da_adesso:".LastController::MAX_AL_NUMBER;

      // valido_dal entro 20 gg da adesso ("A Natale non posso fare un last minute che parte ad Agosto !!!")
      $rules["valido_dal"][] = "entro_giorni_da_adesso:".LastController::MAX_DAL_NUMBER;      
  ...
  private function _add_messages(&$messages)
    {
      ...
      // valido_al - valido_dal <= 1 MESI 
      $messages["valido_al.entro_giorni_da_adesso"] = "La data di fine soggiorno non può superare ".LastController::MAX_AL_NUMBER. " giorni dalla data di inizio offerta";

      // valido_dal - valido_al  <= 45 giorni 
      $messages["valido_al.range_validita_da_giorno_dal"] = "La data di fine soggiorno non può superare " . LastController::MAX_AL_NUMBER . " giorni dalla data di inizio validità";
      
      // valido_dal entro 20 gg da adesso ("A Natale non posso fare un last minute che parte ad Agosto !!!")
      $messages["valido_dal.entro_giorni_da_adesso"] = "La data di inizio soggiorno non può superare ".LastController::MAX_DAL_NUMBER. " giorni da oggi";
      ...
      $messages["tipologia.switch_from_last_to_offer"] = "Non è possibile salvare una nuova offerta per RAGGIUNTO NUMERO MASSIMO (" . Hotel::LIMIT_OFFERTE . ")";
    ...
    if ($this->request->has('offerta_id')) 
      {
        ...
        $rules = [
                ...
                "tipologia" => ["switch_from_last_to_offer:$hotel_id"]
               ];
        ...
        foreach (Utility::linguePossibili() as $lang) 
          {
            ...
            $rules['titolo'.$key] = ["required","max_character:".constant("App\Http\Controllers\Admin\LastController::$const_titolo")];
            $rules['testo'.$key] = ["required","offer_message_spam", "max_character:".constant("App\Http\Controllers\Admin\LastController::$const_testo")];
    ...
    else 
      {
        ...
        /////////////////////////////
        $rules =  [
                "titolo" => ["required","max_character:".LastController::LIMIT_TITOLO],
                "valido_dal" => ["required", "date_format:d/m/Y"], 
                "valido_al" => ["required", "date_format:d/m/Y"],
                "prezzo_a_persona" => ["required","integer","between:1,3000"],
                "testo" => ["required","offer_message_spam","max_character:".LastController::LIMIT_TESTO]
                ];  
      ...
      if ($this->request->has('offerta_id')) 
          ...
          foreach (Utility::linguePossibili() as $lang) 
            {
              ...
              $messages['titolo'.$key.'.max_character'] = "Il titolo in " .Utility::getLanguage($lang)[0]." può contenere al massimo " .constant("App\Http\Controllers\Admin\LastController::$const_titolo"). " caratteri";

              $messages['testo'.$key.'.required'] = "Il testo in " .Utility::getLanguage($lang)[0]." è obbligatorio";
              
              $messages['testo'.$key.'.max_character'] = "Il testo in " .Utility::getLanguage($lang)[0]." può contenere al massimo ". constant("App\Http\Controllers\Admin\LastController::$const_testo") ." caratteri";

              $messages['testo'.$key.'.offer_message_spam'] = "Il testo dell'offferta in " .Utility::getLanguage($lang)[0]." NON deve contenere INDIRIZZI EMAIL, INDIRIZZI INTERNET o NUMERI DI TELEFONO";
      ...
      else
        {
          ...
          $messages =  [
                  ...
                  "titolo.max_character" => "Il titolo può contenere al massimo ".LastController::LIMIT_TITOLO." caratteri",
                  "testo.required" => "Il testo è obbligatorio",
                  "testo.max_character" => "Il testo può contenere al massimo ".LastController::LIMIT_TESTO." caratteri",
